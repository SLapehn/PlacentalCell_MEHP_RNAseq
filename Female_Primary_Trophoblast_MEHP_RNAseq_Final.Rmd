---
title: "Primary_Trophoblast_MEHP_RNAseq Female Final"
...
### Analysis of Female Primary Trophoblast MEHP Samples

This code performs filtering and normalization on Female primary trophoblast  samples that were treated in the Paquette Lab and sequenced by Novogene. There are 12 total female samples including 2 full datasets of (1uM MEHP, 90uM MEHP, 180uM MEHP and DMSO), one partial dataset (missing 1uM). For this reason the sex-stratified analysis will only include the higher dose groups (MEHP 90uM and MEHP 180uM) for a total of 9 samples.
...
```{r load data}
SampleInfo<-read.csv("/Volumes/paquette_a/slapehn/PrimaryTrophoblast_Phthalate_RNAseq/Female_PrimaryTrophoblast_Final/SampleInfo_Female_Final.csv")
...
```{r import prepare}
dir<-"/Volumes/paquette_a/slapehn/PrimaryTrophoblast_Phthalate_RNAseq/Data/Kallisto_Female"
...
Shape<-car::recode(Shape, "'19_15'='15'; '19_78'='16'; '21_20'='17'")
...
```{r Save Data}
save(y_Final,LogCPM,annot,file="/Volumes/paquette_a/slapehn/PrimaryTrophoblast_Phthalate_RNAseq/Female_PrimaryTrophoblast_Final/NormalizedData.Rdata")
```
...
MakeVPlot(AllResults_MEHP_90uM,"DEGs: 90uM MEHP Female")  
MakeVPlot(AllResults_MEHP_180uM,"DEGs: 180uM MEHP Female")
...
bardf$Dose <- factor(bardf$Dose, levels=c("90uM MEHP", "180uM MEHP"))

bardf %>% ggplot(aes(x=Dose, y=Count, fill=Direction)) + 
  geom_bar(stat="identity", position="dodge") + 
  theme(text = element_text(size=15)) + 
  ggtitle("Female Primary Cell- MEHP DEG Direction by Dose") + 
  geom_text(position=position_dodge(1), aes(label=Count)) + 
  theme_bw() + 
  scale_fill_manual(values=c("#FCC3FC", "#F996F9")) + 
  ylim(0, 20) + 
  ylab("Number of DEGs")
```
...
MEHP_90_and_180 %>% 
  ...
  geom_point() + 
  ggtitle("Female MEHP 90uM and 180uM Concordance") + 
  xlab("logFC 90uM") + 
  ylab("logFC 180uM") 
```
...
Performed pathway analysis using fry with KEGG pathways after removing disease-associated pathways. 
...
#No Pathways Significant

```
## Compare to CANDLE Results
Compare DEG lists to results of the human CANDLE evaluation of phthalates and the placental transcriptome (Paquette et al. 2021, EHP)
...
Sig_CANDLE_Primary_MEHP <- inner_join(Sig_CANDLE_MEHP, Sig_MEHP_90_180, by=c("Gene"="hgnc_symbol"), suffix=c("_CANDLE", "_Primary"))

#No CANDLE Overlap with Female Primary Results
```

## lncRNA Sig Results
Identify if any of the DEGs are lncRNAs
```{r lncRNA}
